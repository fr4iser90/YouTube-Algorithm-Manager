...
export const ExtensionBridge = forwardRef<ExtensionBridgeHandle, ExtensionBridgeProps>(({
  ...
  const setupEventListeners = () => {
    // Listen for BroadcastChannel messages (cross-domain)
    try {
      ...
      channel.addEventListener('message', (event) => {
        ...
        switch (event.data.type) {
          case 'EXTENSION_ALIVE':
            // Extension is sending alive signals
            const aliveData = event.data.data;
            setExtensionStatus(prev => ({
              ...prev,
              isInstalled: true,
              isConnected: true,
              version: aliveData.version,
              isTraining: aliveData.isTraining || false,
              detectionMethod: 'broadcast-channel'
            }));
            break;
          ...
          case 'TRAINING_ERROR':
            console.error('Extension training error:', event.data.error);
            setExtensionStatus(prev => ({ ...prev, isTraining: false }));
            setTrainingProgress(null);
            break;
    ...
    const handleStorageChange = () => {
      const progressData = localStorage.getItem('yt-trainer-progress');
      const resultsData = localStorage.getItem('yt-trainer-results');
      ...
      if (resultsData) {
        try {
          ...
          onTrainingComplete?.(results);
          localStorage.removeItem('yt-trainer-results'); // Clear after processing
          setTrainingProgress(null);
        } catch (error) {
      ...
      if (extensionData) {
        // Extension has updated status, trigger check
        checkExtensionStatus();
      }
    };
