...
import type { AnalyticsResults, AlgorithmSnapshotAnalyticsProps } from '@/types/algorithm';
import { AlgorithmSnapshotAnalyzer } from './algorithmSnapshotAnalyzer';
...
export const AlgorithmSnapshotAnalytics: React.FC<AlgorithmSnapshotAnalyticsProps> = ({
  onAnalysisComplete,
  onSaveProfile,
  onCreatePreset
}) => {
  ...
  useEffect(() => {
    ...
    const handleMessage = (message: any) => {
      if (message.type === 'PRE_TRAINING_ANALYSIS_COMPLETE') {
        const analyzer = new AlgorithmSnapshotAnalyzer(message.state);
        const analysisResults = analyzer.analyzeContent();
        setResults(analysisResults);
        chrome.storage.local.set({ lastAnalysisResults: analysisResults }); // Save results
  ...
  const saveAsProfile = () => {
    ...
    const newProfilePreset = {
        ...
        language: 'en',
        region: 'US',
        searches: results.topKeywords.slice(0, 5).map((kw: { term: string }) => ({
            query: kw.term,
            frequency: 2,
            duration: 60
        })),
        targetKeywords: results.topKeywords.map((kw: { term: string }) => kw.term),
        avoidKeywords: [],
        trainingDuration: 15,
        advancedOptions: {
            engagementRate: 0.4,
    ...
    const newProfile = {
      ...
      algorithmState: {
        ...
        region: 'US',
        blockedChannels: [],
        prioritizedChannels: results.topChannels.slice(0, 5).map((c: { channel: string }) => c.channel),
        ...
        contentAnalysis: {
          ...
          sentiment: 'neutral',
          category: 'custom',
          keywords: results.topKeywords.map((k: { term: string }) => k.term)
        }
      },
      createdAt: new Date(),
      lastUsed: new Date(),
      ...
      trainingHours: 0,
      isActive: false,
      tags: ['analyzed', ...results.topKeywords.slice(0, 4).map((kw: { term: string }) => kw.term)],
      preferredCategories: Object.keys(results.categoryDistribution),
      preferredChannels: results.topChannels.slice(0, 5).map((c: { channel: string }) => c.channel),
      preferredKeywords: results.topKeywords.map((k: { term: string }) => k.term),
      watchHistory: [],
      avoidedChannels: [],
      avoidedKeywords: []
    };
  ...
  const createPreset = () => {
    ...
    window.dispatchEvent(new CustomEvent('open-preset-editor', { detail: { preset } }));
