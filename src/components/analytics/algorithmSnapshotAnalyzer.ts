...
export class AlgorithmSnapshotAnalyzer {
  ...
  private extractTopKeywords(): { term: string; score: number }[] {
    ...
    this.state.recommendations.forEach((rec: { title: string }) => {
  ...
  private analyzeChannels() {
    ...
    this.state.recommendations.forEach((rec: { channel: string; category: string; title: string }) => {
      ...
      rec.title.toLowerCase().split(/\s+/).forEach((word: string) => {
  ...
  private getTopVideos() {
    return this.state.recommendations
      .slice(0, 10)
      .map((rec: { title: string; id: string }) => ({
        title: rec.title,
  ...
  private calculateCategoryDistribution(): Record<string, number> {
    ...
    this.state.recommendations.forEach((rec: { category: string }) => {
