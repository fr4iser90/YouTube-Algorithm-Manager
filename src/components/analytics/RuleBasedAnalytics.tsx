...
import { RuleBasedContentAnalyzer, ContentAnalysis, createContentAnalyzer } from '@/components/analytics/ruleBasedContentAnalyzer';

interface RuleBasedAnalyticsProps {
  isActive: boolean;
  recommendations: any[];
...
export const RuleBasedAnalytics: React.FC<RuleBasedAnalyticsProps> = ({
  isActive,
  recommendations,
  targetKeywords,
  avoidKeywords
}) => {
  const [analyzer, setAnalyzer] = useState<RuleBasedContentAnalyzer | null>(null);
  ...
  useEffect(() => {
    if (recommendations.length > 0 && analyzer && isInitialized) {
      analyzeRecommendations();
    }
  }, [recommendations, analyzer, isInitialized]);

  const initializeClassifier = async () => {
    try {
      const mlClassifier = createContentAnalyzer();
      await mlClassifier.initialize();
      
      setAnalyzer(mlClassifier);
  ...
  const analyzeRecommendations = async () => {
    if (!analyzer || recommendations.length === 0) return;
    ...
    try {
      ...
      const results = await analyzer.analyzeMultipleContents(contents);
      setAnalyses(results);
    ...
    <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-lg font-semibold text-white">Rule-Based Content Analytics</h3>
          {isInitialized && (
        ...
        <div className="flex items-center space-x-3">
          <div className="text-xs text-gray-400">
            Analysis Accuracy: {Math.round(modelMetrics.accuracy * 100)}%
          </div>
          {isAnalyzing && (
      ...
      {!isInitialized ? (
        <div className="text-center py-8">
          ...
          <p className="text-gray-400">Initializing Content Analyzer...</p>
        </div>
      ) : analyses.length === 0 ? (
        <div className="text-center py-8">
          <Target className="h-12 w-12 text-gray-500 mx-auto mb-3" />
          <p className="text-gray-400">Start automation to see content analysis</p>
        </div>
      ) : (
